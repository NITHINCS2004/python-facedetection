...
def get_current_location():
    ...
    if datetime.now() - last_location_time > timedelta(minutes=1):  # Update location every 1 minute
        try:
            ...
            if loc:
                location = loc.split(',')
                cached_location = float(location[0]), float(location[1])
...
def is_within_area(lat, lon, center_lat, center_lon, radius_km):
    device_location = (lat, lon)
    center_location = (center_lat, center_lon)
    return distance(device_location, center_location).km <= radius_km

def markAttendance(name, center_lat, center_lon, radius_km):
    current_lat, current_lon = get_current_location()
    if current_lat is None or current_lon is None or not is_within_area(current_lat, current_lon, center_lat, center_lon, radius_km):
...
center_lat = 12.965750  # Example: Latitude of Bengaluru city center
center_lon = 77.589600  # Example: Longitude of Bengaluru city center
    ...
    for encodeFace, faceLoc in zip(encodeCurFrame, facesCurFrame):
        ...
        if matches[matchIndex]:
            ...
            if name not in attendance_marked:
                markAttendance(name, center_lat, center_lon, radius_km)
                attendance_marked.add(name)

    current_time = datetime.now()
    for name, ((x1, y1, x2, y2), timestamp) in list(display_faces.items()):
        if (current_time - timestamp).seconds > 5:  # Remove face display after 5 seconds
